...
<section class="py-3">
    <div class="container">
        <div class="row">
            <h1 class="h3">Listado de países</h1>
            <p class="lead">Se listan todos los países existentes en la base de datos</p>
            <div>
                <a href="#" class="btn btn-success"
                   data-bs-toggle="modal" data-bs-target="#modalAgregarPais">Agregar país</a>
            </div>
        </div>
        <c:choose>
            <c:when test="${listaPaises != null && !lista.isEmpty()}">
                <div class="row g-4 mt-3 row-cols-1 row-cols-sm-2 row-cols-lg-3 row-cols-xl-4 row-cols-xxl-5" data-masonry='{"percentPosition": true }' >
                    <jsp:include page="WEB-INF/pages/paises/partes/cardsPaises.jsp"/>
                </div>
            </c:when>
            <c:otherwise>
                <div class="row mt-4">
                    <div class="col-12">
                        <p class="display-5 text-danger">Ooops! Parece que no hay paises...</p>
                    </div>
                </div>
            </c:otherwise>
        </c:choose>
...
<jsp:include page="WEB-INF/pages/paises/partes/modalAgregarPais.jsp" />
